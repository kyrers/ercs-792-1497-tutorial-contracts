...
contract CentralizedArbitratorWithAppeal is IArbitrator {
    ...
    ) public payable override returns (uint disputeID) {
    ...
    function executeRuling(uint256 _disputeID) public {
        ...
        if (block.timestamp <= dispute.appealPeriodEnd) {
            revert BeforeAppealPeriodEnd(
                block.timestamp,
                dispute.appealPeriodEnd
            );
        }
    ...
    function appealPeriod(
        uint256 _disputeID
    ) public view override returns (uint256 start, uint256 end) {
        Dispute storage dispute = disputes[_disputeID];
